...
#         seq_f = self.data.iloc[index]['F primer']
#         seq_p = self.data.iloc[index]['probe']
#         seq_r = self.data.iloc[index]['R primer']
#         seq_ps = self.data.iloc[index]['Product Sequence']
...
# def collate_CNN_fn(data):
    
#     data.sort(key=lambda x: len(x[0]), reverse=True)

#     seqs, targets = zip(*data)

#     onehot = one_hot_encode(seqs)
#     target = merge_reg(targets)
...
#     onehot = torch.zeros(len(seqs), len(base_dict), max(lengths)).float()
...
class CollateCNN:
    ...
    #     seqs, targets = zip(*data)
        if self.is_test:
            ...
            seqs, targets = zip(*data)
            onehot = self.one_hot_encode(seqs)
        #     onehot_p, lengths_p = one_hot_encode(seq_p)
        #     onehot_r, lengths_r = one_hot_encode(seq_r)
        #     onehot_ps, lengths_ps = one_hot_encode(seq_ps)
        #     seqs = (onehot, lengths_f, onehot_p, lengths_p, onehot_r, lengths_r, onehot_ps, lengths_ps)
